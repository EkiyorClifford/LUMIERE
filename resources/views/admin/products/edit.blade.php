...
@section('content')
<div class="page active" id="page-add-product"><div class="page-header"><div><div class="page-eyebrow">CATALOGUE</div><h1 class="page-title">Edit Product</h1></div></div>
<form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data" class="form-layout">@csrf @method('PUT')
    ...
    <div class="field form-full"><label class="field-label">CURRENT IMAGES</label>
      <div class="grid grid-cols-4 gap-3 mb-4">
        @foreach($product->images as $image)
          <div class="relative">
            <img src="{{ $image->image_url }}" class="w-full h-32 object-cover rounded-sm">
            ...
            <div class="mt-2">
              <input class="lux-input" type="file" name="replace_images[{{ $image->id }}]" accept="image/*">
            </div>
          </div>
        @endforeach
      </div>
  ...
  <div class="form-sidebar-card"><div class="form-sidebar-section"><div class="form-sidebar-title">PUBLISHING</div><label class="toggle-wrap"><input type="checkbox" name="is_active" value="1" @checked(old('is_active', $product->is_active))><span class="toggle-label">Active on storefront</span></label></div><div class="form-sidebar-section"><button class="btn btn-gold" type="submit"><i class="fa-solid fa-check"></i> SAVE CHANGES</button></div></div>
</form></div>
@endsection
